...
module.exports = class ReportBuilder {
    ...
    addSkipped(result) {
        ...
        const comment = formattedResult.suite.skipComment;

        this._skips.push({suite: formattedResult.suite.fullName, browser: formattedResult.browserId, comment});
    ...
    save() {
        const reportDir = this.pluginConfig.path;
    ...
    getResult() {
        const {defaultView, baseHost} = this.pluginConfig;
