...
const getReferenceAbsoluteUrl = (testResult, baseImagesUrl, stateName) => {
    const referenceImagePath = getReferencePath(testResult, stateName);

    return url.resolve(baseImagesUrl, referenceImagePath);
};

const getCurrentAbsoluteUrl = (testResult, baseImagesUrl, stateName) => {
    const currentImagePath = getCurrentPath(testResult, stateName);

    return url.resolve(baseImagesUrl, currentImagePath);
};

const getDiffAbsoluteUrl = (testResult, baseImagesUrl, stateName) => {
    const diffImagePath = getDiffPath(testResult, stateName);

    return url.resolve(baseImagesUrl, diffImagePath);
};
...
function moveImageAsync(srcPath, destPath) {
    return makeDirFor(destPath)
        .then(() => fs.move(srcPath, destPath, {overwrite: true}));
}
...
function getImagesFor(status, formattedResult, {stateName, baseImagesUrl = ''} = {}) {
    ...
    if (status === SUCCESS || status === UPDATED) {
        return {expectedImg: {path: getReferenceAbsoluteUrl(formattedResult, baseImagesUrl, stateName), size: refImg.size}};
    }

    if (status === FAIL) {
        return {
            expectedImg: {
                path: getReferenceAbsoluteUrl(formattedResult, baseImagesUrl, stateName),
                size: refImg.size
            },
            actualImg: {
                path: getCurrentAbsoluteUrl(formattedResult, baseImagesUrl, stateName),
                size: currImg.size
            },
            diffImg: {
                path: getDiffAbsoluteUrl(formattedResult, baseImagesUrl, stateName),
                size: {
    ...
    if (status === ERROR) {
        return {
            actualImg: {
                path: formattedResult.state ? getCurrentAbsoluteUrl(formattedResult, baseImagesUrl, stateName) : '',
